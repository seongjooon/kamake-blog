...
JS에서 scope란 런타임 중에 코드의 특정 부분에 있는 변수, 함수 및 객체에 대한 접근 가능성을 말한다. 즉 scope는 코드에서 변수 및 기타 리소스의 가시성을 결정한다. -최소한의 접근 원칙
...
for (var i = 0; i < 3; i++) {
  printsToBeExecuted.push((ii => () => console.log(ii))(i))
}
...
변수 또는 함수 선언을 찾아야하는 경우 먼저 현재 범위를 살펴보고 상위 범위까지 길게 유지한다 그것이 필요한 선언을 찾지 못했기 때문이다. 갈 수있는 가장 높은 수준은 _글로벌 범위_ 이다.
